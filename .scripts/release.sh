...
if [ "$GENERATE_NOTES" = false ]; then
    ...
    if git rev-list "$LATEST_TAG..HEAD" --count | grep -q "^0$"; then
        ...
        echo "CI pipeline will build the release at: https://github.com/siganberg/ncsender-plugin-pendant/actions"
        exit 0
    fi
...
PROMPT="Based on the following git commit messages from $LATEST_TAG to HEAD, generate user-focused release notes for version $NEW_VERSION of the Pendant plugin.
